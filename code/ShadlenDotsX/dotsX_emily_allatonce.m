function [frames,rseed,start_time,end_time,response,response_time] = dotsX_emily(screenInfo,dotInfo,dat,letterInd,letterColor)
...
for df = 1 : dotInfo.numDotField
    ...
    ss{df} = rand(ndots(df), 2); % array of dot positions raw [x,y]
...
% draw fixation
draw_fixation_circle(curWindow,dat);
if ~drawLetters
...
while continue_show
    ...
    if continue_show < round(dotInfo.maxDotTime*screenInfo.monRefresh)
    ...
    for df = 1:dotInfo.numDotField
        ...
        % NaN dots within fixation radius
        %inCircle = sqrt(xyDis(1,:).^2 + xyDis(2,:).^2) + dotInfo.dotSize/2 <= 2*dat.stm.fixationRadiusPix;  
        %dots2Display(:,inCircle) = NaN;
    ...
    % draw letters
    if drawLetters
        if GetSecs - startTrial <= dat.letter_sec
            if letterColor
                DrawFormattedText(curWindow, dat.letter_set(letterInd), 'center', 'center', [repmat(dat.stm.fixationIntensity+dat.stm.letterIntensityDifference,1,3) 100]);
                %Screen('DrawText', curWindow, dat.letter_set(letterInd) ,dat.scr.x_center_pix, dat.scr.y_center_pix, dat.stm.fixationIntensity+dat.stm.letterIntensityDifference, dat.stm.fixationIntensity)
            else
                DrawFormattedText(curWindow, dat.letter_set(letterInd), 'center', 'center', [repmat(dat.stm.fixationIntensity-dat.stm.letterIntensityDifference,1,3) 100]);
            end
        end
    ...
    if dat.save_stim
        ...
        imageArrayAll = [imageArrayAll {imageArray}];
        
        % imwrite is a Matlab function, not a PTB-3 function
        %imwrite(imageArray, ['../frames/test_' num2str(frames) '.jpg'])
    end
...
if dat.save_stim
    ...
    for i = 1:length(imageArrayAll)
        
        imwrite(imageArrayAll{i},['../test_frames/im_' num2str(i) '.jpg']);
        ...
        [y, newmap] = cmunique(imageArrayAll{i});
        
        %Creates a .gif animation - makes first frame, then appends the rest
        if i==1
            imwrite(y, newmap, '../test_frames/stimulus.gif');
        else
            imwrite(y, newmap, '../test_frames/stimulus.gif', 'DelayTime', delayTime, 'WriteMode', 'append');
        end
    end
